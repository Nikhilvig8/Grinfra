...
{
    public class BirthHrListController : ApiController
    {
        // GET api/<controller>
        GInfraEntities GInfraEntities = new GInfraEntities();
        public HttpResponseMessage Post([FromBody]BirthdayListRequest1 request)
        {
            BirthdayList1 BirthdayList = new BirthdayList1();
 ...
 UriComponents.SchemeAndServer, UriFormat.Unescaped);
            try
            {
                ...
                if (request.userid != "" && request.userid != null)
                {
                    ...
                    var list = GInfraEntities.sp_hrnotification(request.userid).ToList();
                    if (birthdaylist.Count > 0)
                    {
                        foreach (var item in birthdaylist)
                        {
                            BirthdayListResponse1 BirthdayListResponse = new BirthdayListResponse1();
                    ...
                    if (list.Count > 0)
                    {
                        ...
                        {
                            HrResponse1 HrResponse = new HrResponse1();
